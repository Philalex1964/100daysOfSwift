 ...
 */
// let url1 = URL(string: "https://www.\(website)")!
/*
...
/*
 ...
 What does that that isLuckyNumber() function do? I haven’t included it here because it doesn’t really matter, but it’s still some code that isn’t visible right here so we can’t guarantee that items will have any items by the time we call randomElement() on it.
 */
...
enum Direction1: CaseIterable {
    case north, south, east, west

    static func random() -> Direction1 {
        Direction1.allCases.randomElement()!
    }
}
/*
 With that in place, everywhere we need to get a random direction no longer needs a force unwrap:
 */
let randomDirection1 = Direction1.random()
 ...
 */
let user = username(for: 11) ?? "Anonymous"
/*
...
let surnameLetter2 = names1["Claude"]?.first?.uppercased()
 ...
 */
let surnameLetter3 = names1["Vincent"]?.first?.uppercased() ?? "?"
...
/*
 ...
 For what it’s worth, I use optional try a heck of a lot in my own code, because it does wonders for letting me focus on the problem at hand.If that’s what you wanted to do then you could just make runRiskyFunction() return an optional rather than throwing an error, but throwing and using optional try does give us the flexibility to change our mind later. That is, if we write a function that throws errors then at the call site we can either use try/catch or use optional try based on what we need at that point.
 ...
 */
/*
 You can has as many failing paths as you need, but you don’t need to worry about the success path – if you don’t return nil from the method, Swift assumes you mean everything worked correctly.
